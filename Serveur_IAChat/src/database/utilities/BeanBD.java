...
public class BeanBD {
    ...
    public int connect()
    {
        ...
        else
        {
            ...
            ReadProperties rP ;
            try {
                rP = new ReadProperties("Config.properties");
                String s;
    ...
    public boolean findTicket(String user){
        
        try {
            setRs(getInstruc().executeQuery("Select * from billets"));
            while(getRs().next())
            {
                String s =getRs().getString("idBillets");
                if(s.equals(user))
                    return true;
            }
